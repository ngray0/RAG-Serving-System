...
# Calculate reference distances using torch.cdist with p=2 for L2
# This is a built-in, optimized PyTorch function
    ref_distances_f64 = torch.cdist(X_cpu_f64, A_cpu_f64, p=1)
    rtol = 1e-5  # Relative tolerance
    atol = 1e-6  # Absolute tolerance

# Move GPU result to CPU for comparison
    gpu_distances_cpu_f32 = man_dists.cpu()

# Compare the float32 GPU result against the float64 CPU reference
# torch.allclose handles the dtype difference here
    are_close = torch.allclose(gpu_distances_cpu_f32, (ref_distances_f64).to(torch.float32), rtol=rtol, atol=atol)

    if are_close:
        print("Verification successful Manhattan (PyTorch): GPU results are close to CPU torch.cdist results within tolerance.")
    else:
        print("Verification failed Manhattan (PyTorch): Discrepancies found.")
    # Calculate and print the maximum difference
